 ...
 * @package TYPO3
 * @subpackage tx_news2
 * @version $Id: NewsController.php 42548 2011-01-24 19:04:40Z just2b $
 */
class Tx_News2_Controller_NewsController extends Tx_Extbase_MVC_Controller_ActionController {
	...
	public function archiveMenuAction() {
		...
		// check for archive-restrictions
		$archiveSettings = $this->settings['archive'];

		switch($archiveSettings){
			case 'active':
				$valueToUse = 'getDatetime';
				break;
			case 'archived':
				$valueToUse = 'getArchive';
				break;
			default:
				$valueToUse = 'getDatetime';
				break;
		...
		// get all the news by given restrictions
		$newsRecords = $this->newsRepository->findByArchiveRestrictions();

		$yearAr = array();
		$monthAr = array();

		// iterate the returned news objects and check fo
		foreach($newsRecords as $news){
				
			// get the timestamp of every object
			$timestamp = $news->$valueToUse();
				
			// anzahl der Tage des aktuellen Monats
			//print(date ("t", mktime(0,0,0,$timestamp->format("m"),1,$timestamp->format("Y"))));
			//die();
			...
			// create a unique array of years and according months
			if($this->array_searchRecursive($currentRec['month'],$yearAr[$currentRec['year']],FALSE,NULL) === FALSE){
				$yearAr[$currentRec['year']][] = $currentRec['month'];
			}
		}
		
		$this->view->assign('years', $yearAr);
		//$this->view->assign('news', $newsRecords);
	...
	/**
	 * Output a list view of news by month
	 *
	 * @param string $month
	 * @param string $year
	 *
	 * return Tx_News2_Domain_Repository_NewsRepository news
	 */
	public function listArchiveMenuAction($month, $year) {
		// calculate the interval to query the news
		$startInterval = mktime(0,0,0,$month,1,$year);
		$endInterval = mktime(0,0,0,$month+1,1,$year);
		...
		$newsRecords = $this->newsRepository->findByMonth($startInterval,$endInterval);
		$this->view->assign('news', $newsRecords);
	}
